...
use krikos::{SecretKey, endpoint::PortmapperConfig};
use krikos_runtime::{NoopTraceSink, RootSeed};
use krikos_sim::engine::{
...
fn stage4_environment(c: &mut Criterion) {
    ...
    group.bench_function("production_builder_simulation_hooks_disabled", |b| {
        b.iter(|| black_box(krikos::endpoint::Builder::empty()));
    });
    group.bench_function("production_builder_portmapper_disabled", |b| {
        b.iter(|| {
            black_box(
                krikos::endpoint::Builder::empty().portmapper_config(PortmapperConfig::Disabled),
            )
        });
    });
    group.finish();
...
fn kernel(seed: [u8; 32]) -> (Kernel, Arc<krikos_runtime::RuntimeContext>) {
...
fn discovery() -> (DeterministicDiscovery, krikos::EndpointId) {
