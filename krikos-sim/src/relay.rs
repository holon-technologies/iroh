...
use krikos_runtime::{Clock, ClockSleep, RuntimeContext, TokioClock};
...
impl krikos::simulation::RelayConnector for RelayEnvironment {
    fn connect(
        &self,
        request: krikos::simulation::RelayConnectRequest,
    ) -> Pin<
        Box<
            dyn Future<
                    Output = Result<
                        krikos_relay::client::Client,
                        krikos::simulation::RelayConnectError,
                    >,
                > + Send,
        >,
    > {
        ...
        Box::pin(async move {
            let node = this
                ...
                .get(request.url())
                .cloned()
                .ok_or_else(|| krikos::simulation::RelayConnectError::new("unknown relay URL"))?;
            if !*node.availability.borrow() {
                ...
                while !*availability.borrow_and_update() {
                    availability.changed().await.map_err(|_| {
                        krikos::simulation::RelayConnectError::new("relay lifecycle ended")
                    })?;
                }
            }
            Self::connect_node(
            ...
            .await
            .map_err(|error| krikos::simulation::RelayConnectError::new(error.to_string()))
        })
    }
}
