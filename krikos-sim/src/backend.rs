...
impl DeterministicBackend {
    ...
    pub fn endpoint_environment(
        ...
        if self.crypto_mode == SimulationCryptoMode::DeterministicTest {
            let provider = deterministic_crypto::deterministic_test_crypto_provider(
                krikos_relay::tls::default_provider(),
    ...
    pub fn set_port_mapping(
        ...
        if active {
            ...
            krikos::simulation::PortMapper::deactivate(mapper.as_ref());
        }
...
fn derive_crypto_seed(root: RootSeed, host: &str) -> [u8; 32] {
    let mut hasher =
        blake3::Hasher::new_derive_key("krikos-sim deterministic TLS endpoint seed v1");
