...
pub(super) const fn manifest_crypto_mode(
    mode: krikos::simulation::SimulationCryptoMode,
) -> crate::CryptoMode {
    match mode {
...
) -> krikos::simulation::SimulationCryptoMode {
    match mode {
        crate::CryptoMode::DeterministicTest => {
            krikos::simulation::SimulationCryptoMode::DeterministicTest
        }
...
pub(super) fn fidelity_exceptions(mode: krikos::simulation::SimulationCryptoMode) -> Vec<String> {
    match mode {
        krikos::simulation::SimulationCryptoMode::DeterministicTest => {
            vec!["deterministic_test_crypto".to_owned()]
        }
        krikos::simulation::SimulationCryptoMode::ProductionProvider => Vec::new(),
    }
}

pub(super) fn crypto_escapes(mode: krikos::simulation::SimulationCryptoMode) -> Vec<String> {
    match mode {
        krikos::simulation::SimulationCryptoMode::DeterministicTest => Vec::new(),
        krikos::simulation::SimulationCryptoMode::ProductionProvider => {
...
pub(super) fn normalized_trace_bytes(
    events: &[krikos_runtime::TraceEvent],
) -> Result<Vec<u8>, CliError> {
...
pub(super) fn raw_trace_bytes(events: &[krikos_runtime::TraceEvent]) -> Result<Vec<u8>, CliError> {
    let mut bytes = Vec::new();
    for event in events {
        bytes
...
pub(super) fn first_raw_trace_divergence(
    expected: &[krikos_runtime::TraceEvent],
    actual: &[krikos_runtime::TraceEvent],
) -> usize {
    expected
        .iter()
        .zip(actual)
