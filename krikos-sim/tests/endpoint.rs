...
use krikos_runtime::{RootSeed, UnsafeTestOnly};
use support::{
...
async fn production_endpoints_exchange_a_quic_stream_over_the_synthetic_network() {
    ...
    let backend = DeterministicBackend::new(
        DeterministicBackendConfig {
            ...
            crypto_mode: krikos::simulation::SimulationCryptoMode::DeterministicTest,
        },
        Arc::new(trace.clone()),
    )
    .unwrap();
...
async fn run_ipv6_datagram(seed: [u8; 32]) -> Vec<krikos_runtime::TraceEvent> {
    let trace = TraceBuffer::default();
    let backend = DeterministicBackend::new(
        DeterministicBackendConfig {
