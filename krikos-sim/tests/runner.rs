...
struct ResourceFailureEvidence {
    ...
    trace: Vec<krikos_runtime::TraceEvent>,
...
async fn zero_probability_reordering_is_disabled() {
    ...
    assert!(!trace.iter().any(|event| matches!(
        &event.event,
        krikos_runtime::TraceEventKind::FaultInjected { rule, .. }
...
async fn declarative_scenario_runs_real_quic_and_replays_the_same_report_and_trace() {
    ...
    assert!(
        first_report
            .tasks
            .iter()
            .any(|task| matches!(task.metadata.kind, krikos_runtime::TaskKind::SocketActor))
    );
    assert!(
        first_report
            .tasks
            .iter()
            .any(|task| matches!(task.metadata.kind, krikos_runtime::TaskKind::Noq))
    );
    assert!(first_report.tasks.iter().any(|task| {
        matches!(task.metadata.kind, krikos_runtime::TaskKind::Other(ref kind) if kind == "remote-state-actor")
...
async fn relay_only_scenario_runs_production_quic_client_and_server_sessions() {
    ...
    assert!(report.tasks.iter().any(|task| {
        matches!(task.metadata.kind, krikos_runtime::TaskKind::Relay)
...
async fn interface_down_up_notifies_production_socket_actor_and_preserves_quic() {
    ...
    let interface_events = first_trace
        ...
        .filter(|event| {
            matches!(
                &event.event,
                krikos_runtime::TraceEventKind::InterfaceState { host, .. } if host == "client"
            )
        })
        .count();
    ...
    assert_eq!(
        first_trace
            ...
            .filter(|event| matches!(
                &event.event,
                krikos_runtime::TraceEventKind::InterfaceAddress { host, .. } if host == "client"
            ))
            .count(),
        2
    );
    ...
    assert_eq!(
        first_trace
            ...
            .filter(|event| matches!(
                &event.event,
                krikos_runtime::TraceEventKind::HostPower { host, .. } if host == "client"
            ))
            .count(),
        2
    );
    assert!(first_trace.iter().any(|event| {
        matches!(
            &event.event,
            krikos_runtime::TraceEventKind::PacketCreated { source, .. }
...
async fn declared_stateful_nat_and_firewall_carry_real_quic_and_replay() {
    ...
    assert!(first_trace.iter().any(|event| matches!(
        &event.event,
        krikos_runtime::TraceEventKind::NatMapping { .. }
    )));
    assert!(first_trace.iter().any(|event| matches!(
        &event.event,
        krikos_runtime::TraceEventKind::NatMapping { transition, .. }
    ...
    assert!(first_trace.iter().any(|event| matches!(
        &event.event,
        krikos_runtime::TraceEventKind::NatTranslation { .. }
    )));
    assert!(first_trace.iter().any(|event| matches!(
        &event.event,
        krikos_runtime::TraceEventKind::FirewallDecision { .. }
    )));
    assert_eq!(
        first_trace_divergence(&first_trace, &second_trace).unwrap(),
        None
...
async fn production_connect_aggregates_conflicting_delayed_discovery_and_expires_records() {
    ...
    assert!(first_trace.iter().any(|event| matches!(
        &event.event,
        krikos_runtime::TraceEventKind::DiscoveryRecord { transition, .. }
...
async fn run(
    scenario: Scenario,
    seed: [u8; 32],
) -> (support::ScenarioReport, Vec<krikos_runtime::TraceEvent>) {
    let trace = TraceBuffer::default();
    let runner = ScenarioRunner::deterministic(
        scenario,
        RootSeed::new(seed),
