...
use krikos_blobs::store::GcConfig;
use krikos_docs::{engine::ProtectCallbackHandler, protocol::Docs};
...
#[derive(Debug, Clone)]
pub struct Client {
    blobs: krikos_blobs::api::Store,
    docs: krikos_docs::api::DocsApi,
}

impl Client {
    fn new(blobs: krikos_blobs::api::Store, docs: krikos_docs::api::DocsApi) -> Self {
        Self { blobs, docs }
    }

    pub fn blobs(&self) -> &krikos_blobs::api::Store {
        &self.blobs
    }

    pub fn docs(&self) -> &krikos_docs::api::DocsApi {
        &self.docs
    }
}
...
impl Builder {
    ...
    async fn spawn0(
        ...
        let mut router = krikos::protocol::Router::builder(self.endpoint.clone());
        ...
        router = router.accept(
            krikos_blobs::ALPN,
            krikos_blobs::BlobsProtocol::new(&blobs, None),
        );
        router = router.accept(krikos_docs::ALPN, docs.clone());
        router = router.accept(krikos_gossip::ALPN, gossip.clone());
    ...
    pub async fn spawn(self) -> anyhow::Result<Node> {
        let (store, protect_handler) = match self.storage {
            ...
            Storage::Persistent(ref path) => {
                ...
                let mut opts = krikos_blobs::store::fs::options::Options::new(path);
                ...
                let store = krikos_blobs::store::fs::FsStore::load_with_opts(db_path, opts).await?;
