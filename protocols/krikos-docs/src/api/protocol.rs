...
/// Options to configure what is included in a [`krikos::EndpointAddr`].
#[derive(
    Copy,
    Clone,
    PartialEq,
...
impl AddrInfoOptions {
    ...
    pub fn apply(
        &self,
        krikos::EndpointAddr { id, addrs }: &krikos::EndpointAddr,
    ) -> krikos::EndpointAddr {
        match self {
            Self::Id => krikos::EndpointAddr::new(*id),
            Self::Relay => krikos::EndpointAddr {
                id: *id,
                addrs: addrs
                    .iter()
                    .filter(|addr| matches!(addr, krikos::TransportAddr::Relay(_)))
                    .cloned()
                    .collect(),
            },
            Self::Addresses => krikos::EndpointAddr {
                id: *id,
                addrs: addrs
                    .iter()
                    .filter(|addr| matches!(addr, krikos::TransportAddr::Ip(_)))
                    .cloned()
                    .collect(),
            },
            Self::RelayAndAddresses => krikos::EndpointAddr {
                id: *id,
                addrs: addrs.clone(),
            },
        }
