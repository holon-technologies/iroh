...
use anyhow::Result;
use krikos::{Endpoint, endpoint::Connection, protocol::ProtocolHandler};
...
impl ProtocolHandler for Docs {
    async fn accept(&self, connection: Connection) -> Result<(), krikos::protocol::AcceptError> {
        self.engine
            .handle_connection(connection)
            .await
            .map_err(|err| krikos::protocol::AcceptError::from_err(n0_error::anyerr!(err)))?;
        Ok(())
    }
