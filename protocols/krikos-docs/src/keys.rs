...
use krikos::{KeyParsingError, PublicKey, SecretKey, Signature, SignatureError};
use rand::CryptoRng;
...
///
/// Internally, an author wraps an [`krikos::SecretKey`] used to sign entries.
...
///
/// This is the corresponding [`krikos::PublicKey`] for an author. It is used as an identifier,
...
impl AuthorPublicKey {
    ...
    /// See [`krikos::PublicKey::from_bytes`] for details.
...
/// Internally, a [`NamespaceSecret`] wraps an [`krikos::SecretKey`] used to sign entries.
#[derive(Clone, Serialize, Deserialize)]
...
/// The corresponding [`krikos::PublicKey`] for a [`NamespaceSecret`].
