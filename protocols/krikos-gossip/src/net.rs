//! Networking for the `krikos-gossip` protocol
...
enum ConnectionLoopError {
    ...
    Connection {
        #[error(std_err)]
        source: krikos::endpoint::ConnectionError,
    },
    ActorDropped {},
}
...
struct Dialer {
    ...
    pending: JoinSet<(
        EndpointId,
        Option<Result<Connection, krikos::endpoint::ConnectError>>,
    )>,
...
pub(crate) mod tests {
    ...
    async fn gossip_net_smoke() {
        ...
        let (relay_map, relay_url, _guard) = krikos::test_utils::run_relay_server().await.unwrap();
    ...
    async fn can_die_and_reconnect() -> Result {
        ...
        let (relay_map, _relay_url, _guard) = krikos::test_utils::run_relay_server().await.unwrap();
        let rng = &mut rand::rngs::ChaCha12Rng::seed_from_u64(1);
