...
This directory preserves the `iroh-gossip` source history for the Holon Iroh monorepo.
...
- Operation: `--to-subdirectory-filter protocols/iroh-gossip`
...
The rewrite ran in disposable bare mirror `/tmp/iroh-gossip-import.oOmChh/iroh-gossip.git`, never
...
- all 43 release-tree paths were under `protocols/iroh-gossip/`;
...
- `cargo test --manifest-path protocols/iroh-gossip/Cargo.toml --locked` passed 19 unit tests and
