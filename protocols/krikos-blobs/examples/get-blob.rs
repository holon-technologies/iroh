...
use krikos::{address_lookup::PkarrResolver, endpoint::presets};
use krikos_blobs::{BlobFormat, get::request::GetBlobItem, ticket::BlobTicket};
...
async fn main() -> anyhow::Result<()> {
    ...
    let endpoint = krikos::Endpoint::builder(presets::Minimal)
        .relay_mode(krikos::RelayMode::Default)
    ...
    let connection = endpoint.connect(ticket.addr().id, krikos_blobs::ALPN).await?;
    let mut progress = krikos_blobs::get::request::get_blob(connection, ticket.hash());
