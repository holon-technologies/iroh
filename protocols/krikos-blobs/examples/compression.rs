...
use krikos::{endpoint::presets, protocol::ProtocolHandler};
use krikos_blobs::{
    api::Store,
    get::StreamPair,
    provider::{
...
trait Compression: Clone + Send + Sync + Debug + 'static {
    ...
    fn recv_stream(
        &self,
        stream: krikos::endpoint::RecvStream,
    ) -> impl krikos_blobs::util::RecvStream + Sync + 'static;
    fn send_stream(
        &self,
        stream: krikos::endpoint::SendStream,
    ) -> impl krikos_blobs::util::SendStream + Sync + 'static;
}
...
mod lz4 {
    ...
    use krikos_blobs::util::{
        AsyncReadRecvStream, AsyncReadRecvStreamExtra, AsyncWriteSendStream,
        AsyncWriteSendStreamExtra,
    };
    use tokio::io::{AsyncRead, AsyncWrite, BufReader};

    struct SendStream(Lz4Encoder<krikos::endpoint::SendStream>);

    impl SendStream {
        pub fn new(inner: krikos::endpoint::SendStream) -> AsyncWriteSendStream<Self> {
    ...
    struct RecvStream(Lz4Decoder<BufReader<krikos::endpoint::RecvStream>>);

    impl RecvStream {
        pub fn new(inner: krikos::endpoint::RecvStream) -> AsyncReadRecvStream<Self> {
    ...
    impl super::Compression for Compression {
        const ALPN: &[u8] = concat_const::concat_bytes!(b"lz4/", krikos_blobs::ALPN);
        fn recv_stream(
            &self,
            stream: krikos::endpoint::RecvStream,
        ) -> impl krikos_blobs::util::RecvStream + Sync + 'static {
...
impl<C: Compression> ProtocolHandler for CompressedBlobsProtocol<C> {
    ...
    ) -> std::result::Result<(), krikos::protocol::AcceptError> {
...
async fn main() -> Result<()> {
    ...
    match args {
        Args::Provide { path } => {
            ...
            let router = krikos::protocol::Router::builder(endpoint.clone())
