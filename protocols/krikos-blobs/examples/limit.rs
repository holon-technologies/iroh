...
use krikos::{EndpointAddr, EndpointId, SecretKey, endpoint::presets, protocol::Router};
use krikos_blobs::{
    ...
    match args {
        Args::Get { ticket } => {
            let connection = endpoint
                .connect(ticket.addr().clone(), krikos_blobs::ALPN)
                .await?;
            let (data, stats) = krikos_blobs::get::request::get_blob(connection, ticket.hash())
                .bytes_and_stats()
                .await?;
