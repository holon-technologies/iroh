...
use krikos::endpoint::{ConnectionError, ReadError, VarInt, WriteError};
...
impl GetError {
    pub fn krikos_error_code(&self) -> Option<VarInt> {
        if let Some(ReadError::Reset(code)) = self
            ...
            .and_then(|e| e.downcast_ref::<krikos::endpoint::ReadError>())
        {
            Some(*code)
        } else if let Some(WriteError::Stopped(code)) = self
            .remote_write()
            .and_then(|source| source.get_ref())
            .and_then(|e| e.downcast_ref::<krikos::endpoint::WriteError>())
        {
            Some(*code)
        } else if let Some(ConnectionError::ApplicationClosed(ac)) = self
            .open()
            .and_then(|source| source.get_ref())
            .and_then(|e| e.downcast_ref::<krikos::endpoint::ConnectionError>())
        {
            Some(ac.error_code)
        } else {
            None
