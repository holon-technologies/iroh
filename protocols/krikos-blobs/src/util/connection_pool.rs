...
const POOL_ACTOR_QUEUE_CAPACITY: usize = 100;
const CONNECTION_ACTOR_QUEUE_CAPACITY: usize = 100;
...
impl Deref for ConnectionRef {
    type Target = krikos::endpoint::Connection;
...
impl ConnectionRef {
    fn new(connection: krikos::endpoint::Connection, counter: OneConnection) -> Self {
        Self {
            connection,
            _permit: counter,
        }
...
impl Actor {
    pub fn new(
        ...
        let (tx, rx) = mpsc::channel(POOL_ACTOR_QUEUE_CAPACITY);
        (
            Self {
                rx,
    ...
    async fn handle_msg(&mut self, msg: ActorMessage) {
        match msg {
            ActorMessage::RequestRef(mut msg) => {
                ...
                let (conn_tx, conn_rx) = mpsc::channel(CONNECTION_ACTOR_QUEUE_CAPACITY);
...
mod tests {
    ...
    async fn echo_server() -> TestResult<(EndpointAddr, Router)> {
        ...
        let router = krikos::protocol::Router::builder(endpoint)
