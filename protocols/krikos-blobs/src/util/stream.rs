...
/// An abstract `krikos::endpoint::RecvStream`.
...
impl SendStream for krikos::endpoint::SendStream {
    ...
    async fn stopped(&mut self) -> io::Result<Option<VarInt>> {
        Ok(krikos::endpoint::SendStream::stopped(self).await?)
    }
...
impl RecvStream for krikos::endpoint::RecvStream {
