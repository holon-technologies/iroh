...
|---|---|---|---|---|---|
| Relay only | Production QUIC over the production Krikos relay actor and production relay client/server session handlers | Loopback relay service with OS TCP/TLS/HTTP | Relay-backed endpoint connection | terminal, authentication, delivery, path | Deterministic transport omits listener mechanics, not relay protocol mechanics |
