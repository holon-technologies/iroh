...
fn collect_resource_sample(
    started: Instant,
    previous_cpu: iroh_bench::canary::CpuTicks,
    phase: &'static str,
) -> Result<(ResourceSample, iroh_bench::canary::CpuTicks), String> {
    let current_cpu =
        parse_cpu_ticks(&fs::read_to_string("/proc/stat").map_err(|error| error.to_string())?)
            .map_err(|error| error.to_string())?;
    let memory =
...
fn relay_client_outcome_json(
    outcomes: iroh_bench::canary::workloads::RelayClientOutcomeCounts,
) -> Value {
    json!({
