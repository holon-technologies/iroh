...
fn builder_retains_one_complete_simulation_environment() {
    #[derive(Debug)]
    struct NeverBind(iroh_runtime::ClockDomain);
    ...
    let builder = Builder::empty()
        .simulation_environment_for_test(environment, iroh_runtime::UnsafeTestOnly::acknowledge());
...
async fn explicit_simulation_environment_reaches_bound_endpoint() {
    #[derive(Debug)]
    struct RuntimeOwnedOsSockets(iroh_runtime::ClockDomain);
...
async fn injected_ip_factory_prevents_os_socket_binding() {
    ...
    struct RejectBind {
        ...
        domain: iroh_runtime::ClockDomain,
    }
    ...
    let context = Arc::new(iroh_runtime::RuntimeContext::tokio(
        iroh_runtime::RootSeed::new([37; 32]),
        Arc::new(iroh_runtime::NoopTraceSink),
    ));
    let environment = crate::simulation::SimulationEnvironment::new(
        context.clone(),
