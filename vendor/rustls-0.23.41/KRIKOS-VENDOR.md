...
`krikos-sim` requires deterministic, run-scoped cryptography so that a simulation
...
they violate `krikos-sim`'s run-scoping and no-leak invariants (a leaked or globally dispatched
...
Simulator only. This crate is patched in by `krikos-sim/Cargo.toml`. No
