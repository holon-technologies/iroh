...
pub(super) const MAX_ACME_CACHE_FILE_BYTES: usize = 1024 * 1024;
...
#[derive(Clone, Debug)]
pub(super) struct BoundedAcmeCache {
    root: PathBuf,
}
...
fn cache_file_name(prefix: &str, identifiers: &[String], directory_url: &str) -> String {
    ...
    format!("{prefix}{}", base64_url::encode(&digest.finalize()))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn cache_rejects_the_first_byte_over_the_limit_on_load() {
        let directory = tempfile::tempdir().expect("temporary ACME cache");
        let cache = BoundedAcmeCache::new(directory.path().to_path_buf());
        let contact = ["mailto:test@example.com".to_owned()];
        let directory_url = "https://acme.invalid";
        let path = cache.account_path(&contact, directory_url);
        std::fs::write(&path, vec![0_u8; MAX_ACME_CACHE_FILE_BYTES + 1])
