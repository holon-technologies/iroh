...
/// via `JNI_OnLoad`:
///
/// *Note: `install_android_jni_context` is reexported from `krikos`, so you can substitute
/// `krikos_resolver` for `krikos` below.*
///
/// ```ignore
...
///     unsafe {
///         krikos_resolver::install_android_jni_context(vm, res);
///     }
