...
use krikos_relay::{RelayConfig, quic::QuicClient};
use krikos_relay::{
    RelayMap,
...
enum QadProbeError {
    ...
    Quic { source: krikos_relay::quic::Error },
...
impl Client {
    ...
    pub(crate) fn new(
        ...
        mut opts: Options,
        metrics: Arc<Metrics>,
    ) -> Result<Self, krikos_relay::quic::QuicClientBuildError> {
        ...
        let quic = opts
            ...
            .take()
            .map(|config| {
                krikos_relay::quic::QuicClient::new(config.ep, config.client_config)
...
mod test_utils {
    ...
    use krikos_relay::{RelayConfig, RelayQuicConfig, server};
...
mod tests {
    ...
    async fn test_basic() -> Result<()> {
        ...
        let client_config = krikos_relay::tls::make_dangerous_client_config();
