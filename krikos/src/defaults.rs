//! Default values used in [`krikos`][`crate`]
...
pub use krikos_relay::defaults::DEFAULT_RELAY_QUIC_PORT;
use url::Url;
...
pub mod prod {
    use krikos_relay::{RelayConfig, RelayMap};
...
/// Contains all timeouts that we use in `krikos`.
