...
///
/// Since krikos endpoint are reachable via a relay server we have a network path indicated by
...
impl MappedAddr for RelayMappedAddr {
    ...
    /// which is recognised by krikos as an IP mapped address.
    fn generate() -> Self {
        let mut addr = [0u8; 16];
        addr[0] = ADDR_PREFIXL;
        addr[1..6].copy_from_slice(&ADDR_GLOBAL_ID);
...
/// which map to a unique [`krikos_base::CustomAddr`].
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash, Ord, PartialOrd)]
