use krikos_base::{EndpointId, TransportAddr};
...
impl RemoteInfo {
    ...
    /// # {
    /// # use krikos::{Endpoint, EndpointId, EndpointAddr, endpoint::presets};
    /// # #[tokio::main]
    /// # async fn main() {
    ...
    /// let addr =
    ///     EndpointAddr::try_from_parts(info.id(), info.into_addrs().map(|addr| addr.into_addr()))
    ///         .unwrap();
    /// # }
    /// # }
    /// ```
