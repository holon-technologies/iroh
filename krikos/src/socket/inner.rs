...
pub(crate) struct EndpointInner {
    ...
    actor_task: Mutex<Option<krikos_runtime::OwnedTaskHandle>>,
...
impl EndpointInner {
    ...
    pub(crate) fn runtime_context(&self) -> &Arc<krikos_runtime::RuntimeContext> {
        self.runtime.context()
    }

    #[cfg(all(test, not(wasm_browser)))]
    pub(crate) fn runtime_task_snapshot(&self) -> krikos_runtime::TaskGroupSnapshot {
    ...
    pub(crate) async fn bind(opts: Options) -> Result<Self, BindError> {
        ...
        let actor_task = runtime
            .spawn_owned(
                krikos_runtime::TaskKind::SocketActor,
                "socket-actor",
        ...
        if let Some(task) = task {
            ...
            match crate::runtime::RuntimeTimeout::after(
                ...
                Ok(timeout) => match timeout.await {
                    ...
                    Err(krikos_runtime::TimeoutError::Elapsed) => {
                        warn!("tasks didn't finish in time, aborting");
                    }
                    Err(krikos_runtime::TimeoutError::Clock(error)) => {
    ...
    ///
    /// This should only be called in the `krikos::Endpoint` `Drop` impl when the
    /// `krikos::Endpoint` is dropped without first calling `Endpoint::close`.
    #[instrument(skip_all, parent = self.sock.span.clone())]
