...
#[cfg(not(wasm_browser))]
use krikos_relay::quic::QuicClient;
use krikos_relay::{
    RelayConfig, RelayMap, defaults::DEFAULT_RELAY_QUIC_PORT, http::RELAY_PROBE_PATH,
};
#[cfg(not(wasm_browser))]
use krikos_resolver::{DnsError, DnsResolver, StaggeredError};
...
mod tests {
    ...
    async fn test_qad_probe_v4() -> Result {
        ...
        let quic_client = krikos_relay::quic::QuicClient::new(ep.clone(), client_config)?;
