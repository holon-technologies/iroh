...
//!
//! To connect to an krikos endpoint a [`EndpointAddr`] is needed, which may contain a
...
//! information.  Each krikos endpoint will automatically publish their own addressing
...
//! ```
//!
//! [`EndpointAddr`]: krikos_base::EndpointAddr
...
use krikos_base::{EndpointAddr, EndpointId};
pub use krikos_dns::{ParseError, endpoint_info::AddrFilter};
...
///
/// Krikos uses this trait to allow configuring the set of address lookup services on
...
mod tests {
    ...
    use krikos_base::{EndpointAddr, SecretKey, TransportAddr};
...
mod test_dns_pkarr {
    use krikos_base::{EndpointAddr, SecretKey, TransportAddr};
    use krikos_dns::{dns::EndpointDnsResolver, endpoint_info::UserData};
    use krikos_relay::tls::{CaTlsConfig, default_provider};
    ...
    async fn ep_with_address_lookup<R: rand::CryptoRng + ?Sized>(
        rng: &mut R,
        relay_map: &krikos_relay::RelayMap,
