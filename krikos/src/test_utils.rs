...
pub async fn run_relay_server_with_access(
    ...
    let (_certs, server_config) = krikos_relay::server::testing::self_signed_tls_certs_and_config();
...
pub(crate) mod pkarr_relay {
    ...
    async fn pkarr_put(
        ...
        let signed_packet = krikos_dns::pkarr::SignedPacket::from_relay_payload(&key, &body)
