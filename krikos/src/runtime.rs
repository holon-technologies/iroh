...
use krikos_runtime::{OwnedTaskHandle, RuntimeContext, SpawnError, TaskGroup, TaskKind};

/// Adapts the shared wall clock to Noq's token-validity clock.
#[cfg(not(wasm_browser))]
#[derive(Debug)]
pub(crate) struct NoqWallClock(Arc<dyn krikos_runtime::WallClock>);

#[cfg(not(wasm_browser))]
impl NoqWallClock {
    pub(crate) fn new(clock: Arc<dyn krikos_runtime::WallClock>) -> Self {
        Self(clock)
    }
}
...
impl Runtime {
    ...
    pub(crate) fn new_with_limits(
        ...
        let tasks = context.executor().new_group_with_limits(
            None,
            krikos_runtime::TaskGroupLimits::new(limits.max_live_tasks()),
        );
        Self {
            id,
            context,
    ...
    pub(crate) fn task_capacity_snapshot(&self) -> crate::endpoint::TaskCapacitySnapshot {
        match self.tasks.capacity_snapshot() {
            krikos_runtime::TaskGroupCapacitySnapshot::Reported {
                max_live_tasks,
                live_tasks,
            ...
            krikos_runtime::TaskGroupCapacitySnapshot::Unreported => {
                crate::endpoint::TaskCapacitySnapshot {
                    maximum: 0,
                    current: 0,
                    high_water: 0,
    ...
    #[cfg(all(test, not(wasm_browser)))]
    pub(crate) fn task_snapshot(&self) -> krikos_runtime::TaskGroupSnapshot {
        self.tasks.snapshot()
    }
    ...
    pub(crate) fn spawn(
        ...
        name: &str,
        future: Pin<Box<dyn std::future::Future<Output = ()> + Send + 'static>>,
    ) -> Result<krikos_runtime::TaskId, SpawnError> {
...
#[cfg(not(wasm_browser))]
pub(crate) type RuntimeInterval = krikos_runtime::ClockInterval;

#[cfg(not(wasm_browser))]
pub(crate) type RuntimeSleep = krikos_runtime::ClockSleep;

#[cfg(not(wasm_browser))]
pub(crate) type RuntimeTimeout<F> = krikos_runtime::ClockTimeout<F>;
...
#[derive(Debug)]
struct NoqTimer {
    timer: Option<Pin<Box<dyn krikos_runtime::Timer>>>,
    failure: Arc<std::sync::Mutex<Option<String>>>,
}
...
mod tests {
    ...
    impl krikos_runtime::WallClock for FixedWallClock {
