...
use krikos::{Endpoint, EndpointAddr, endpoint::presets};
...
mod remote_map {
    ...
    impl Aggregate {
        fn update(&mut self, addr: TransportAddr, stats: krikos::endpoint::PathStats) {
    ...
    impl RemoteMap {
        ...
        async fn clear_expired(
            retention_time: Duration,
            map: Arc<RwLock<HashMap<krikos::PublicKey, RemoteInfo>>>,
        ) {
