...
pub(crate) fn is_relayed(conn: &krikos::endpoint::Connection) -> bool {
    conn.paths()
        .iter()
        .find(|p| p.is_selected())
        .expect("no selected path")
...
fn endpoint_builder(device: &Device, relay_map: RelayMap) -> krikos::endpoint::Builder {
    #[allow(unused_mut)]
    ...
    #[cfg(feature = "qlog")]
    {
        if let Some(path) = device.filepath("qlog") {
            ...
            let transport_config = krikos::endpoint::QuicTransportConfig::builder()
