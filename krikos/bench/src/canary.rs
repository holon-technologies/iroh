...
impl WorkloadProfile {
    ...
    pub fn production_twice() -> Result<Self, CanaryError> {
        let dns = krikos_dns_server::config::LimitsConfig::default();
        let relay = krikos_relay::server::Limits::default();
        let endpoint = krikos::endpoint::EndpointLimits::default();
        ...
        let relay_accept_rate_per_second = integral_rate(
            ...
            krikos_relay::server::DEFAULT_ACCEPT_CONN_LIMIT,
        )?;
        let relay_accept_burst = required_nonzero(
            "relay_accept_conn_burst",
            krikos_relay::server::DEFAULT_ACCEPT_CONN_BURST,
        )?;
