...
async fn run_relay_phases(
    ...
    phases: &PhaseReporter,
    server: &RelayServer,
    relay_url: &krikos::RelayUrl,
    tls_config: &rustls::ClientConfig,
    metrics: &krikos_relay::server::Metrics,
...
fn rejection_total(metrics: &krikos_relay::server::Metrics) -> LaneResult<u64> {
    metrics
        .admission_rate_limited
        .get()
...
async fn run_dns_phases(
    ...
    metrics: &krikos_dns_server::Metrics,
...
fn dns_http_connection_rejections(metrics: &krikos_dns_server::Metrics) -> LaneResult<u64> {
    metrics
