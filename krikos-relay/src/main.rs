...
use krikos_relay::server::AcmeConfig;
use krikos_relay::{
    defaults::{
...
impl TryFrom<AccessConfig> for Arc<dyn krikos_relay::server::DynAccessControl> {
    type Error = AnyError;

    fn try_from(cfg: AccessConfig) -> Result<Self> {
        match cfg {
            AccessConfig::Everyone => Ok(Arc::new(krikos_relay::server::AllowAll)),
...
struct TlsConfig {
    ...
    ///
    /// Defaults to the `https_bind_addr` with the port set to [`krikos_relay::defaults::DEFAULT_RELAY_QUIC_PORT`].
    ///
    /// If `https_bind_addr` is not set, defaults to `http_bind_addr` with the
    /// port set to [`krikos_relay::defaults::DEFAULT_RELAY_QUIC_PORT`]
